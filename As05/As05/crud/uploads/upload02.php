...
$fileExists = false;
$sql = "SELECT filename FROM tutors WHERE filename='$fileName'";
...
if ($result) {
    ...
    $sql = "UPDATE tutors set filename = ?, filetype = ?, filesize = ? ,description = ? WHERE id = ?";
    
    $q = $pdo->prepare($sql);
    $q->execute(array($fileName,$fileType,$fileSize,$fileDescription,$tutorid));
